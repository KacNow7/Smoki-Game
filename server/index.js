...
function swapDreamCards(room, playerAId, slotA, playerBId, slotB) {
...
function findDreamCard(room, playerId, slotIndex) {
    const player = getPlayer(room, playerId);
    return player?.dream[slotIndex] ?? null;
}
function findDreamSlotByCardId(room, playerId, cardId) {
    const player = getPlayer(room, playerId);
    if (!player) {
        return -1;
    }
...
function setTopOpenCard(room, pileIndex, card) {
    const idx = pileIndex ?? 0;
    room.openPiles[idx] ??= [];
    room.openPiles[idx].push({ ...card, isFaceUp: true });
}
function removeTopOpenCard(room, pileIndex) {
    const idx = pileIndex ?? 0;
...
function drawCard(room, source, openPileIndex) {
    if (source === 'OPEN') {
...
function placeCardInDream(room, playerId, slotIndex, card) {
    const player = getPlayer(room, playerId);
    if (!player) {
        return null;
    }
...
function discardCardToOpenPile(room, card, pileIndex) {
    setTopOpenCard(room, 0, card);
}
function handleCircleCard(room, playerId, slotIndex, replacedCard) {
    const player = getPlayer(room, playerId);
...
function maybeEnterScoring(room) {
...
function finalizeScoringIfReady(room) {
...
function canCurrentPlayerAct(room, socketId) {
...
io.on('connection', (socket) => {
    ...
    socket.on('joinRoom', (roomIdInput, callback) => {
    ...
    socket.on('drawCard', ({ source, openPileIndex }) => {
    ...
    socket.on('discardPendingDraw', ({ openPileIndex }) => {
        const room = getRoomForSocket(socket);
    ...
    socket.on('keepPendingDraw', ({ slotIndex }) => {
        const room = getRoomForSocket(socket);
        if (!room || room.currentTurnPlayerId !== socket.id || room.turnPhase !== 'CHOOSE_KEEP_OR_DISCARD' || !room.pendingDraw) {
            return;
        }
    ...
    socket.on('attackSwapCard', ({ ownSlotIndex, opponentSlotIndex }) => {
        const room = getRoomForSocket(socket);
    ...
    socket.on('swapNestCards', ({ firstSlotIndex, secondSlotIndex }) => {
        const room = getRoomForSocket(socket);
        const player = getPlayer(room ?? undefined, socket.id);
    ...
    socket.on('submitMirrorChoices', ({ choices }) => {
        const room = getRoomForSocket(socket);
    ...
    socket.on('chooseInitialReveal', ({ slotIndex }) => {
        const room = getRoomForSocket(socket);
        if (!room || room.turnPhase !== 'CHOOSE_INITIAL_REVEAL')
            return;
    ...
    socket.on('continueRound', () => {
        ...
        if (room.roundEnderPlayerId !== socket.id && room.nextStartingPlayerId !== socket.id) {
            return;
        }
